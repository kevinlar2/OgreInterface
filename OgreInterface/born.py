...
class BornRepulsionCalculator(PairPotential):
    ...
    # @property
    def get_potentials(self, r0=2, n=6):
    ...
    # def reset(self):
    #     super().reset()
    #     self.dict = defaultdict(lambda: self.get_potentials().cutoff)
    #     self.df = defaultdict(lambda: self.get_potentials().derivative(1))
    #     self.df2 = defaultdict(lambda: self.get_potentials().derivative(2))

    def calculate(self, atoms, properties, system_changes):
        """Calculate system properties."""
        # print('in calculate', self.atoms)
        # TODO: Use atoms to set the pairs i.e super().__init__({pair: self.short_range} for pair in pairs)
        ...
        f_dict = {pair: self.get_potentials(r0, 6) for pair, r0 in zip(pairs, r0s)}
...
if __name__ == "__main__":
    ...
    brc = BornRepulsionCalculator(
        cutoff=10.0,
        r0_dict={"Pb": 2.4, "Br": 1.68, "Cs": 4.5, "S": 1.68}
    )
