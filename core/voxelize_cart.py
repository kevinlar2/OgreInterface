...
def generate_unit_cell_tensor(lattice, grid_size=0.1):
    cube_size = np.floor(np.linalg.norm(lattice, axis=1) / grid_size).astype(np.int16)
    unit_cell = np.zeros((cube_size[0], cube_size[1], cube_size[2]), dtype=bool)
    X, Y, Z = np.meshgrid(
        np.arange(cube_size[0], dtype=np.int16),
        np.arange(cube_size[1], dtype=np.int16),
        np.arange(cube_size[2], dtype=np.int16),
    )
...
def get_radii(lattice, radius, grid_size=0.1):
    ...
    #  cube_size = np.floor(np.array([a, b, c]) / grid_size).astype(np.int16)
    r_a = np.floor(radius / grid_size).astype(np.int16)
    r_b = np.floor(radius / grid_size).astype(np.int16)
    r_c = np.floor(radius / grid_size).astype(np.int16)
    print(r_a, r_b, r_c)
...
def get_atoms_volume(unit_cell, lattice):
    ...
    side_length = np.array(unit_cell.shape)
